import { SPOTIFY_ACCESS_TOKEN, SPOTIFY_CODE_VERIFIER, SPOTIFY_REFRESH_TOKEN } from "../utils/constants";
import { fetcher } from "../utils/fetcher";
import { getLocalStorage, setLocalStorage } from "../utils/localStorage";
import type { SpotifyPlaylist, SpotifyTokens, SpotifyUser, SpotifyUserPlaylists } from "../types";
...
class SpotifyService {
    ...
    async getTokens(code: string): Promise<void> {
        ...
        const response = await fetcher<SpotifyTokens>("https://accounts.spotify.com/api/token", {
            method: "POST",
            headers: {
        ...
        if (response.access_token) setLocalStorage(SPOTIFY_ACCESS_TOKEN, response.access_token);
        if (response.refresh_token) setLocalStorage(SPOTIFY_REFRESH_TOKEN, response.refresh_token);
    }
    ...
    async GetUserPlaylists(): Promise<SpotifyUserPlaylists> {
        ...
        let playlists: SpotifyUserPlaylists = {
            ...
            previous: "",
            total: 0,
            items: [{ collaborative: false, description: "", external_urls: { spotify: "" }, href: "", id: "", images: [{ url: "", height: 0, width: 0 }], name: "", owner: { external_urls: { spotify: "" }, followers: { href: "", total: 0 }, href: "", id: "", type: "", uri: "", display_name: "" }, public: false, snapshot_id: "", tracks: { href: "", total: 0 }, type: "", uri: "" }]
        };

        let next: string | null = 'https://api.spotify.com/v1/me/playlists?limit=50';

        while (next) {
            const response: SpotifyUserPlaylists = await fetcher<SpotifyUserPlaylists>(next, {
                method: "GET",
                headers: {
                    "Authorization": `Bearer ${token}`
                }
    ...
    async GetPlaylistDetails(playlistId: string): Promise<SpotifyPlaylist | null> {
        const token = getLocalStorage(SPOTIFY_ACCESS_TOKEN);
        if (!token) throw new Error("No access token available");

        return fetcher<SpotifyPlaylist>(`https://api.spotify.com/v1/playlists/${playlistId}`, {
            method: "GET",
            headers: {
                "Authorization": `Bearer ${token}`
            }
    ...
    async backupPlaylist(playlistId: string): Promise<{ name: string, csv: string }> {
    ...
    private async getPlaylistTracks(playlistId: string): Promise<SpotifyPlaylist["tracks"]["items"]> {
        const token = getLocalStorage(SPOTIFY_ACCESS_TOKEN);
        if (!token) throw new Error("No access token available");
        ...
        while (next) {
            const response: SpotifyPlaylist["tracks"] = await fetcher<SpotifyPlaylist["tracks"]>(next, {
                method: "GET",
                headers: {
                    "Authorization": `Bearer ${token}`
                }
